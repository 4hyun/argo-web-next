...
function buildTreeFromLocation(item, location, root) {
  ...
  if (typeof location === 'number' && location > -1) {
    root.push({ ...item, sort: location, items: [] });
    return;
  }
      ...
      locationKey.split(KEY_DELIMITER).reduce((acc, parentId, i, arr) => {
        ...
        if (Array.isArray(acc)) {
          ...
          itemOrPlaceholder = isLocationKeyEnd
            ? { ...item, sort: location[locationKey] }
            : { id: parentId };
...
export function parseMenuTreeMap(menu) {
  if (!menu)
    throw new Error(
  ...
  items.forEach(item => {
    if (item[LOCATION_FIELD][menuId]) {
      ...
      buildTreeFromLocation(item, location, root);
    }
  });
  // console.log('>>DEBUG: parseMenuTreeMap(?)/root: ', root);
  return root;
}

function parseMenuTreeList(root) {
  if (!root)
    throw new Error(
      `parseMenuTree() Error: invalid 'root' value provided: ${root}. Valid root node object with \`label\`, \`items\` is required.`,
    );

  console.log('>> DEBUG/ parseMenuTreeList()/root :', root);
  /* TODO: [START] */
  if (root.items) {
    root.items.forEach(traverse);
  }
}

export default parseMenuTreeList;
