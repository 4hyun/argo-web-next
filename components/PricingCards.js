...
const DesktopLicenseFeatureHighlightList = (props) => <LicenseFeatureHighlightList {...props} />
...
const Card = ({ priceData, addInquiryItem, showForm }) => {
  const { id, price, price_unit, heading, feature, cta_text, cta_link, desc_list } = priceData
  ...
  return (
    <CardWrapper>
      ...
      <DesktopLicenseFeatureHighlightList featureList={featureList} twClasses="hidden xl:block" />
      <AllFeatureList>
...
const PricingCards = ({ priceList, addInquiryItem, removeInquiryItem, showForm }) => {
  return (
    <Container>
      {priceList &&
        priceList.map((priceData, i) => {
          return <Card key={i} priceData={priceData} addInquiryItem={addInquiryItem} removeInquiryItem={removeInquiryItem} showForm={showForm} />
        })}
    </Container>
  )
}
